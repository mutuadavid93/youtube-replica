      ...
      <form @submit.prevent="addVideo">
        <div>
          ...
          <input
            v-model="title"
            type="text"
            class="form-control block w-full px-5 py-1.5 text-xl font-normal text-gray-200 bg-black placeholder-gray-400 bg-clip-padding border border-solid border-gray-600 rounded transition ease-in-out m-0 focus:ring-0"
...
defineProps({
  // NOTE: This prop contains errros from the backend thus we can as well display them to user.
  // To be exact from $request->validate().
  errors: Object,
});
...
const addVideo = () => {
  ...
  // TIP: This way we don't have to define field names explicitly
  data.append("title", title.value);
  data.append("image", image.value);
  data.append("video", video.value);

  // TIP: Inertia's router can as well post directly to Laravel Routes.
  router.post("/films", data);
};

// TIP: DOM Element's Dynamic values can be assigned to state.
